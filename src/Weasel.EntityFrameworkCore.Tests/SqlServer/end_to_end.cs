...
public class end_to_end : IAsyncLifetime
{
        ...
        _host = Host.CreateDefaultBuilder()
            .ConfigureServices(services =>
            {
                services.AddDbContext<SqlServerDbContext>(options =>
                    options.UseSqlServer(SqlServerDbContext.ConnectionString));

                services.AddSingleton<Migrator, SqlServerMigrator>();
            })
            .Build();
    ...
    public async Task can_create_migration_and_apply()
    {
        ...
        migration.Migrator.ShouldBeOfType<SqlServerMigrator>();
    ...
    public void can_build_a_database_for_db_context()
    {
        ...
        database.Tables.Single().Identifier.Name.ShouldBe("MyEntities");
    }
}

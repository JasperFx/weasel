...
**Superseded in numbering by [9.30.0](/release-9-30), which is the same code.** 9.29.1 shipped three
fixes, one of which — character column widths being compared on MySQL, SQL Server and Oracle —
changes what an existing database does on its next migration. That belonged behind a minor version
rather than a patch, so it was republished as 9.30.0.

Nothing is wrong with 9.29.1 and nothing is different in 9.30.0 but the version. Prefer 9.30.0, and
read [Upgrading to 9.30](/release-9-30) for what the three fixes actually do.

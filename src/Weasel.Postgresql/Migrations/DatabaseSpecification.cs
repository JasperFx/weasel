...
public class DatabaseSpecification
{
    ...
    public string ToCreateDatabaseStatement(string databaseName)
    {
        ...
        if (Encoding != null)
        {
            withs.Add($" ENCODING = '{Encoding}'");
        }

        if (Owner != null)
        {
            withs.Add($" OWNER = '{Owner}'");
        }

        if (ConnectionLimit.HasValue)
        {
            withs.Add($" CONNECTION LIMIT = {ConnectionLimit}");
        }

        if (LcCollate != null)
        {
            withs.Add($" LC_COLLATE = '{LcCollate}'");
        }

        if (LcType != null)
        {
            withs.Add($" LC_CTYPE = '{LcType}'");
        }

        if (TableSpace != null)
        {
            withs.Add($" TABLESPACE  = {TableSpace}");
        }
        ...
        foreach (var with in withs) builder.Append(with);

...
public class end_to_end : IAsyncLifetime
{
        ...
        _host = Host.CreateDefaultBuilder()
            .ConfigureServices(services =>
            {
                services.AddDbContext<MySqlDbContext>(options =>
                    options.UseMySql(MySqlDbContext.ConnectionString, serverVersion));

                services.AddSingleton<Migrator, MySqlMigrator>();
            })
            .Build();
    ...
    public async Task can_create_migration_and_apply()
    {
        ...
        migration.Migrator.ShouldBeOfType<MySqlMigrator>();
